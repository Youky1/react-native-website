...
有些时候，一些动画值需要依赖另一些值来做计算。一个例子是倒置屏幕缩放比例（2x --> 0.5x）
...
例如，你可能想通过使用 `Animated.Value` 的值从0变化到1来让 `position` 从150px变化到0px，同时 `opacity` 从 0 变为 1。但这一点可以通过将 `style` 从example修改为下面的样子来做到：
